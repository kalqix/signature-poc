...
use shared::{
    BatchEthWitness, BatchSepticOptWitness, BatchSepticSingleWitness, BatchSepticVerifyWitness,
    BatchSepticWitness, EthOrderWitness, OrderWitness, P256OrderWitness, ProgramInput,
    ProofOutput, RegisterKeyWitness, SessionKeyLeaf, eth_order_message, order_message,
    p256_order_message, register_key_message,
...
fn handle_batch_septic_single(w: BatchSepticSingleWitness) {
    ...
    for witness in &w.orders {
        ...
        // SEPTIC_SCALAR_MUL precompile: 1 syscall per scalar mul.
...
fn handle_batch_septic_opt_single(w: BatchSepticOptWitness) {
    ...
    // Phase 3 — single 217-bit scalar mul on G via SEPTIC_SCALAR_MUL.
    ...
    // Phase 4 — Σ(ae_i · A_i) via SEPTIC_SCALAR_MUL precompile.
    ...
    // Phase 6 — Σ(α_i · R_i) via SEPTIC_SCALAR_MUL. The full 8-limb α_i is
    // passed even though only the low 128 bits are nonzero — the loop runs
    // inside the executor regardless.
...
fn handle_batch_septic_verify(w: BatchSepticVerifyWitness) {
    ...
    for witness in &w.orders {
        ...
        // Single syscall: Shamir's trick computes s·G + e·A internally.
